...
class TestCreatePost:
    ...
    def test_create_post_title_max_length(self, page: Page, base_url: str):
        # Test title max 100 chars (assuming 100 is enforced in UI/backend)
        long_title = 'A' * 101
        page.goto(base_url)
        page.fill("#title", long_title)
        page.fill("#body", "Body")
        page.get_by_role("button", name="Create Post").click()
        # Should show error or not create post
        wait_for_toast(page, "required")
...
class TestEditPost:
    ...
    def test_edit_post_validation(self, page: Page, base_url: str):
...
class TestSearch:
    ...
    def test_search_by_title(self, page: Page, base_url: str):
        title = unique_text("FilterTitle")
        create_post(page, base_url, title, "Body")
        page.goto(base_url)
        page.fill("input[name='search']", title)
        page.keyboard.press("Enter")
        expect(page.locator(".post-card", has_text=title).first).to_be_visible()
...
class TestListSorting:
    def test_newest_edited_first(self, page: Page, base_url: str):
        ...
        page.goto(base_url)
        # Edit first memo to make it most recently updated
        card = page.locator(".post-card", has_text=title1).first
        card.get_by_role("link", name="Edit").click()
        page.fill("#body", "Updated Body")
        page.get_by_role("button", name="Save Changes").click()
        page.goto(base_url)
        # The edited post should now be first
        first_card = page.locator(".post-card").first
        expect(first_card).to_have_text(title1)
...
class TestSingleUser:
    def test_no_authentication_required(self, page: Page, base_url: str):
        page.goto(base_url)
        expect(page).to_have_title("MemoBoard")
        # Should be able to create a post without login
        create_post(page, base_url, unique_text("NoAuth"), "Body")
        wait_for_toast(page, "Post created successfully!")
